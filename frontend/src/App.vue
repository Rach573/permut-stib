...
type View = 'login' | 'register' | 'pending' | 'home' | 'permutation-menu' | 'signature-menu' | 'new-permutation' | 'permutations' | 'new-signature' | 'signatures' | 'available-signatures' | 'signature-availabilities'
...
onMounted(async () => {
  ...
  if (import.meta.env.DEV && new URLSearchParams(location.search).get('preview') === 'agent') {
  ...
  <main class="shell">
    ...
    <template v-else>
      ...
      <section v-else-if="view === 'available-signatures'" class="stack">
        <button class="secondary" @click="view = 'signature-menu'">← Signatures</button><h1>Signatures recherchées</h1>
      ...
      <section v-else class="stack">
        <button class="secondary" @click="view = 'signature-menu'">← Signatures</button>
